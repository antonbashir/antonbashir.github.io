...
Я помню времена, когда столько нужно было для 3D игру со всеми ресурсами.
...
На этом с Dart SDK мы закончили, давайте сделаем паузу и подумаем над вот таком вопросом. 
...
Для меня это самая приятная часть всего моего пути. Моделирование интефрейса - это чистое творчество.
...
> [!NOTE]
> Бонус! Вообще, эту функцию я добавил буквально в процессе написания статьи. Она вроде бы работает, но на GitHub я её не стал выкладывать
>
...
Потянулись, разомнулись и скоро начнем... 
...
Для начала я бы хотел кратко пояснить про основные способы реализаии "особых" функцих в Dart. Если вкратце:
...
Дело в том, что в Dart существует два представления объектов. Это связано с тем, что Dart использует технику [тегированных указателей](https://github.com/dart-lang/sdk/blob/main/runtime/docs/gc.md):
...
Разница в том, что `recycle` переведт корутину в состояние `disposed`, но не уничтожит её объект и память, выделенную под стек. Вторая функция полностью уничтожает корутину и связанный с ней стек. 
...
Расшифрую: каждый раз, когда мне нужно изменить поля или функции в объекте корутинры, я вынужден обновлять эти файлы. 
...
3. Если доходим до функции иницализации основной корутины (`CoroutineInitialize`), то выполняем обработку изначального (нативного) стека
...
Самый смешной файлик в моем списке. Помните, выше я говорил про `vm:recognized`, так вот такие методы нужно дублировать в этот файл, но все не так просто. 
...
Путь выдался напряженным, но теперь можно расслабиться. На самом деле все закончится хорошо, в следующей части я расскажу о том, как мне пришлось переписать мои любимые корутинчики, чтобы удовлетворить Dart GC. 
